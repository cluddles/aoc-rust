...
fn build_dict(src_patterns: &[String]) -> Vec<Option<u8>> {
    ...
    let d5 = add_to_dict(src_patterns, &mut dict, 5, |x| {
        x.len() == 5 && contains(x, &tl_mid)
    });
    // Six segments: 0, 6, 9
    ...
    let d0 = add_to_dict(src_patterns, &mut dict, 0, |x| {
        x.len() == 6 && contains(x, &tr)
    });
    ...
    add_to_dict(src_patterns, &mut dict, 2, |x| {
        x.len() == 5 && contains(x, &bl)
    });
