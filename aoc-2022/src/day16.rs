...
fn part2(network: &Network) -> u32 {
    ...
    for i in 0..count {
        ...
        best = p2_one(network, &all, &split, num_evals).max(best);
    }
    best
}

fn p2_one(network: &Network, all: &[&String], split1: &[&String], num_evals: &mut u32) -> u32 {
    let mut split2 = all.to_vec();
    split2.retain(|x| !split1.contains(x));
    // println!("({}): s1 {:?}, s2 {:?}", num_evals, split1, split2);
    eval(network, &String::from("AA"), split1, 0, 26, 0, 0, num_evals) + eval(network, &String::from("AA"), &split2, 0, 26, 0, 0, num_evals)
}
