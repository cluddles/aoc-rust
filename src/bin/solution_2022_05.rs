...
fn parse_crates(content: &str) -> Vec<String> {
    let lines = shared::split_lines(content);
    let cols = lines[lines.len()-1].split(' ').last().unwrap().parse::<usize>().unwrap();
    let mut result = vec![String::from(""); cols];
...
fn parse_input(content: &str) -> (Vec<String>, Vec<Move>) {
    let parts: Vec<&str> = content.split("\n\n").collect();
    (parse_crates(parts[0]), parse_moves(parts[1]))
}

fn summarise(crates: &[String]) -> String {
    crates
        .iter()
...
fn part1(crates: &Vec<String>, moves: &Vec<Move>) -> String {
    //println!("{:?}", crates);
    //println!("{:?}", moves);
...
fn part2(crates: &Vec<String>, moves: &Vec<Move>) -> String {
    let mut state = crates.to_owned();
    for m in moves {
        let mut popped = Vec::new();
        for _ in 0..m.quantity {
            popped.push(state[m.from].pop().unwrap());
        }
        popped.reverse();
        for p in popped {
            state[m.to].push(p);
...
mod tests {
    ...
    fn gen_input() -> (Vec<String>, Vec<Move>) {
